	...
	public String performPost(HttpServletRequest request) {
        ...
        try {
        	...
        	for (int i = 0; i<allCameRequests.length;i++) {
        		System.out.println("Come to accept");
        		if (allCameRequests[i].getUserName().equals(form.getUserName()) && allCameRequests[i].getFriendsUserName().equals(form.getFriendsUserName())) {
        			Friends acceptedFriend = new Friends();
        			acceptedFriend.setUserName(form.getFriendsUserName());
        			acceptedFriend.setFriendsUserName(form.getUserName());
        			acceptedFriend.setRequestAccepted(true);
        			acceptedFriend.setRequestDeclined(false);
        			acceptedFriend.setRequestSend(true);
        			try {
        				friendsDAO.delete(allCameRequests[i].getId());
        				friendsDAO.create(acceptedFriend);
